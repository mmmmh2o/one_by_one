...
class _TextStatsPageState extends ConsumerState<TextStatsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return ToolScaffold(
      toolId: 'text_stats',
      children: [
        AppCard(child: TextField(controller: _controller, minLines: 6, maxLines: 10, decoration: const InputDecoration(hintText: '输入或粘贴文本', border: InputBorder.none), onChanged: notifier.setInput)),
  ...
  Widget _row(BuildContext context, SettingsState ui, String label, String value) => Padding(padding: const EdgeInsets.only(bottom: AppSpacing.sm), child: Row(children: [
    Expanded(child: Text(label, style: scaledTextStyle(Theme.of(context).textTheme.bodyMedium, ui.textScaleFactor))),
    Text(value, style: scaledTextStyle(Theme.of(context).textTheme.titleMedium, ui.textScaleFactor)),
  ]));
}
