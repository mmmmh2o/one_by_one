...
class _RandomStringPageState extends ConsumerState<RandomStringPage> {
  ...
  @override
  void initState() { super.initState(); _lengthController = TextEditingController(text: '16'); }
  @override
    ...
    return ToolScaffold(
      ...
      children: [
        AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          AppInput(label: '长度 (1-256)', controller: _lengthController, keyboardType: TextInputType.number, onChanged: (v) => notifier.setLength(int.tryParse(v) ?? 16)),
          const SizedBox(height: AppSpacing.md),
          Wrap(spacing: AppSpacing.sm, runSpacing: AppSpacing.sm, children: [
            FilterChip(label: const Text('小写'), selected: state.useLower, onSelected: notifier.setUseLower),
            FilterChip(label: const Text('大写'), selected: state.useUpper, onSelected: notifier.setUseUpper),
            FilterChip(label: const Text('数字'), selected: state.useDigits, onSelected: notifier.setUseDigits),
            FilterChip(label: const Text('符号'), selected: state.useSymbols, onSelected: notifier.setUseSymbols),
          ]),
        ])),
        ...
        Wrap(spacing: AppSpacing.sm, children: [
          ActionChip(label: const Text('长度 8'), onPressed: () { _lengthController.text = '8'; notifier.setLength(8); }),
          ActionChip(label: const Text('长度 16'), onPressed: () { _lengthController.text = '16'; notifier.setLength(16); }),
        ...
        AppCard(child: state.error != null
            ...
            : SelectableText(state.output.isEmpty ? '等待生成结果' : state.output, style: scaledTextStyle(Theme.of(context).textTheme.bodyLarge, ui.textScaleFactor))),
      ],
    );
  }
}
