...
class _TextComparePageState extends ConsumerState<TextComparePage> {
  ...
  @override
  void initState() { super.initState(); _leftController = TextEditingController(); _rightController = TextEditingController(); }
  @override
  void dispose() { _leftController.dispose(); _rightController.dispose(); super.dispose(); }
  ...
  Widget build(BuildContext context) {
    ...
    return ToolScaffold(
      ...
      expandBody: true,
      children: [
        Expanded(child: AppCard(child: TextField(controller: _leftController, minLines: 6, maxLines: null, expands: true, decoration: const InputDecoration(hintText: '文本 A（每行一条）', border: InputBorder.none), onChanged: notifier.setLeft))),
        const SizedBox(height: AppSpacing.sm),
        Expanded(child: AppCard(child: TextField(controller: _rightController, minLines: 6, maxLines: null, expands: true, decoration: const InputDecoration(hintText: '文本 B（每行一条）', border: InputBorder.none), onChanged: notifier.setRight))),
        ...
        AppCard(child: state.result == null
            ...
            : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: AppSpacing.sm),
                _PreviewChips(title: '仅 A 预览', values: state.result!.onlyA.toList()..sort()),
                const SizedBox(height: AppSpacing.sm),
                _PreviewChips(title: '仅 B 预览', values: state.result!.onlyB.toList()..sort()),
                const SizedBox(height: AppSpacing.sm),
                _PreviewChips(title: '共同项预览', values: state.result!.both.toList()..sort()),
              ])),
      ],
    );
  }
...
class _PreviewChips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title, style: Theme.of(context).textTheme.bodyMedium), const SizedBox(height: AppSpacing.xs),
      if (preview.isEmpty) Text('无', style: Theme.of(context).textTheme.bodySmall)
      else Wrap(spacing: AppSpacing.xs, runSpacing: AppSpacing.xs, children: [for (final item in preview) InputChip(label: Text(item), onPressed: () {})]),
    ]);
  }
}
